  ...
  return (
    <section className="relative overflow-hidden">
      ...
      <div className="absolute inset-0">
        <img src={boardwalkImg} alt="Path to beach" className="w-full h-full object-cover" loading="lazy" width={1920} height={1080} />
      ...
      <div className="container mx-auto px-6 py-32 md:py-40 relative z-10">
        <div ref={ref} className={`reveal ${isVisible ? "visible" : ""}`}>
          <h2 className="font-hebrew text-4xl md:text-6xl font-bold text-foreground text-center mb-6">
            איך זה עובד?
          </h2>
        ...
        <div className={`grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto reveal-stagger ${isVisible ? "visible" : ""}`}>
          {steps.map((step) => (
            <div key={step.num} className="text-center group card-luxury p-10 rounded-2xl bg-card/90 backdrop-blur-sm border border-border/50">
              <span className="font-display text-6xl font-bold text-secondary/40 block mb-6 group-hover:text-primary/50 transition-colors duration-500">
                {step.num}
              </span>
              <h3 className="font-hebrew text-xl font-bold text-foreground mb-4">{step.title}</h3>
