        ...
        <div className="container mx-auto px-6">
          ...
          <div className={`grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-4xl mx-auto reveal-stagger ${isVisible ? "visible" : ""}`}>
            {cards.map((card) => (
              <div
                key={card.title}
                className="card-luxury p-7 rounded-3xl bg-card border border-border/60 hover:border-primary/30"
              >
                <span className="text-3xl block mb-4">{card.icon}</span>
                <h3 className="font-hebrew text-xl font-bold text-foreground mb-2 tracking-wide">{card.title}</h3>
